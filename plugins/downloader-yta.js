...
let handler = async (m, { conn, text, args, command }) => {
	...
	try {
		...
		if (sizeh > 10) throw `Filesize: ${audio.fileSizeH}\nTidak Dapat Mengirim, Maksimal File 10 MB`
		...
		try {
			...
			if (!somematch(['kB','KB'], vsize) && parseInt(anu.link.size.replace(" MB", "")) > 10) return m.reply(`Filesize: ${anu.link.size}\nTidak Dapat Mengirim, Maksimal File 10 MB`)
			...
			try {
				...
				if (!somematch(['kB','KB'], vsize) && parseInt(anu.size.replace(" MB", "")) > 10) return m.reply(`Filesize: ${anu.size}\nTidak Dapat Mengirim, Maksimal File 10 MB`)
