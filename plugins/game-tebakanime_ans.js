...
handler.before = async function (m) {
    ...
    if (m.quoted.id == this.tebakanime[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase()) {
            global.db.data.users[m.sender].limit += this.tebakanime[id][2]
            await conn.sendButton(m.chat, `*Jawabanmu BENAR!🥳🎉*\n_kamu mendapatkan ${this.tebakanime[id][2]} Coin_`, wm, null, buttontebakanime, m)
